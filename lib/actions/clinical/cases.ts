...
/**
 * Case queries take their own scope union: the hub variant also matches
 * cases the clinical lead created themselves (clinicalLeadId), which can
 * have no student and therefore no hub.
 */
export type CasesScope = { hubId: string | null; clinicalLeadId: string } | { projectId: string };
...
export async function fetchClinicalCasesChartData(scope: CasesScope) {
...
/**
 * The cases list joins students/schools/hubs for every scope (unlike the
 * chart queries), so cases without a student never appear here. profileId
 * marks the caller's own cases via the isClinicalLeadCase flag.
 */
export async function fetchClinicalCasesList(
  scope: CasesScope,
  profileId: string,
