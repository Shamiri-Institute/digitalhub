...
export default function UploadRecordingDialog({ open, onOpenChange }: UploadRecordingDialogProps) {
  ...
  // Dropdown data state
  const [fellows, setFellows] = useState<HubFellow[]>([]);
  ...
  useEffect(() => {
    if (fellowId) {
      ...
      // Check if fellow has a supervisor
      const selectedFellow = fellows.find((f) => f.id === fellowId);
      if (selectedFellow && !selectedFellow.supervisorId) {
        toast({
          title: "Warning",
          description:
            "This fellow does not have an assigned supervisor. Recording upload requires a supervisor.",
          variant: "destructive",
        });
      }
  ...
  // Check if file upload is allowed (also check fellow has supervisor)
  const selectedFellow = fellows.find((f) => f.id === fellowId);
  const fellowHasSupervisor = selectedFellow?.supervisorId != null;
  const canUploadFile =
    fellowId &&
    groupId &&
    sessionId &&
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {fellowId && selectedFellow && !fellowHasSupervisor && (
              <div className="rounded-lg border border-red-border bg-red-bg p-3 text-sm text-red-base">
                <div className="flex items-center gap-2">
                  <Icons.alertCircle className="h-4 w-4" />
                  <span>This fellow does not have an assigned supervisor</span>
                </div>
              </div>
            )}
