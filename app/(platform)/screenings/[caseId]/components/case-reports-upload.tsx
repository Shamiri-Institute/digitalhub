...
export function CaseReports({ currentcase }: { currentcase: CurrentCase }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const resp = await fetch("/api/files/gdrive/case-report", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <>
      {currentcase.caseReport ? (
        <div>
          <p className="text-sm text-gray-500">Case Report Link</p>
          <Link
            href={currentcase.caseReport ?? "#"}
            target="_blank"
