...
| `update`/`delete`/`findUniqueOrThrow` threw when no row matched                   | Drizzle returns zero rows                                                                                                   | `.returning()` length checks with explicit throws (many sites)                                                  | keep; a tiny helper would remove the repetition (ENG-2161 decides) |
...
| `_count: { relation }` and filtered counts                                        | no equivalent in RQB v1                                                                                                     | correlated count subqueries in `extras`, rebuilt into `_count` objects for the shared component types           | temporary; ENG-2161 flattens to `xCount`                           |
...
| Raw `COUNT(*)` as BigInt, `AVG`/numeric as Decimal-ish strings                    | pg returns int8 as string and numeric as string                                                                             | `int8 → Number` parser in `db/client.ts`; `::float8` casts in SQL where callers expect numbers                  | parser is temporary (ENG-2161); casts keep                         |
...
| `include` returned `Date` for timestamps, UTC for `timestamp without time zone`   | pg parses naive timestamps as local time                                                                                    | Drizzle's own `mode: "date"` mapping for queries; type parsers for raw rows                                     | parsers temporary (ENG-2161)                                       |
...
| Implicit enum objects and model types from `@prisma/client`                       | 221 importing files                                                                                                         | `db/enums.ts` (plain objects), `db/types.ts`                                                                    | enums keep; `db/types.ts` temporary (ENG-2161)                     |
...
| `queryRaw`/`executeRaw` and pg type parsers                                      | `db/client.ts`                                                                                                                                                                                                                                             | keep raw rows Prisma-shaped during coexistence; removed by ENG-2161                                                               |
...
- Prisma threw on `update`/`delete` of a missing row; Drizzle returns zero rows. Every converted
  ...
  `updateOne(table, set, where)` helper would remove the repetition once Prisma is gone (ENG-2161
  decides).
...
- `fetchDayFellowAttendances` builds `IN (...)` from filter keys and breaks on an empty list; use
